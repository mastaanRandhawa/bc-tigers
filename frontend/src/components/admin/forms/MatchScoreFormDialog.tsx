  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    if (
      ...
      form.setError('home_penalties', {
        message: 'Enter unequal penalty shootout totals before completing this knockout match.',
      });
      return;
    }
      ...
      {showPenalties && (
        <div className="space-y-2 rounded-lg border border-border/60 bg-muted/30 p-3">
          ...
          <p className="text-xs text-muted-foreground">
            Regulation ended level. Enter successful kicks for each team before marking completed.
          </p>
          <div className="grid grid-cols-2 gap-3">
            <TextInputField
          ...
          {form.formState.errors.home_penalties?.message && (
            <p className="text-xs text-destructive">{form.formState.errors.home_penalties.message}</p>
          )}
        </div>
      )}
      {match.is_elimination && regulationTied && watchedStatus !== 'COMPLETED' && (
        <p className="text-xs text-muted-foreground">
          Penalty shootout results are required when completing a tied knockout match.
        </p>
      )}
