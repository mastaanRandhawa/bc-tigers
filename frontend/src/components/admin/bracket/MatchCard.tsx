...
function matchStatus(node: BracketNode): { label: string; tone: 'pending' | 'ready' | 'live' | 'done' } {
  if (node.winner_id) return { label: 'Complete', tone: 'done' };
  if (node.match?.status === 'LIVE' || node.match?.status === 'HALFTIME') return { label: 'Live', tone: 'live' };
  if (node.home_team_id && node.away_team_id) return { label: 'Ready', tone: 'ready' };
...
export function MatchCard({
  ...
  const nodeReady = !node.status || node.status === 'READY' || node.status === 'IN_PROGRESS';
  const canPickWinner =
    nodeReady &&
  ...
  const renderSlot = (slot: 'home' | 'away') => {
    ...
    const isClickTarget = !!selectedTeamId && allowPlacement && !isByeSlot(node, slot);
    const isWinner = !!node.winner_id && node.winner_id === (slot === 'home' ? node.home_team_id : node.away_team_id);
    ...
    if (team) {
      return (
        <MatchTeamRow
          ...
          slot={slot}
          nodeId={node.id}
          score={slot === 'home' ? node.match?.home_score : node.match?.away_score}
          isWinner={isWinner}
          isLoser={isLoser}
          locked={slotLocked}
  ...
  return (
    <m.article
      ...
      className={cn(
        ...
        isDecided ? 'border-emerald-500/25 shadow-[var(--shadow-md)]' : 'border-border/80',
    ...
    >
      <header className="flex items-center justify-between gap-2 border-b border-border/70 bg-muted/30 px-3 py-2">
        <div className="flex items-center gap-1.5 min-w-0">
          {isFinal && <Trophy className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-hidden />}
        ...
        <span
          className={cn(
            ...
            status.tone === 'done' && 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300',
            status.tone === 'ready' && 'bg-primary/10 text-primary',
            status.tone === 'live' && 'bg-amber-500/10 text-amber-700 dark:text-amber-300',
      ...
      {canPickWinner && (
        <footer className="border-t border-border/60 bg-muted/20 px-3 py-2">
          <p className="text-[10px] text-muted-foreground">Click a team to advance them</p>
        </footer>
      )}
    </m.article>
  );
