  ...
  const handleDrop = async (nodeId: string, slot: 'home' | 'away') => {
    ...
    try {
      await updateNodeMutation.mutateAsync({
        nodeId,
        data: slot === 'home'
          ? { home_team_id: dragState.teamId }
          : { away_team_id: dragState.teamId },
      });
    } catch (err) {
                ...
                <div className="flex flex-wrap gap-2">
                  {poolTeams.map((team) => (
                    <div
                      ...
                      className="flex cursor-grab items-center gap-1.5 rounded-md border border-border bg-card px-2.5 py-1.5 text-xs font-medium text-foreground shadow-sm hover:border-primary/40 active:cursor-grabbing select-none"
...
function TeamSlot({ team, isOver, isDragging, isWinner, onDragOver, onDragLeave, onDrop, onRemove }: TeamSlotProps) {
  return (
    <div
    ...
    >
      {team ? (
        ...
        <span className={`text-[11px] italic ${isOver ? 'text-primary' : 'text-muted-foreground/60'}`}>
